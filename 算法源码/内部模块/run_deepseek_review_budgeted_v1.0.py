# -*- coding: utf-8 -*-
"""Budgeted guard for DeepSeek priority review.

Default mode is dry-run: estimate rows, batches, token-like volume, and cost,
then write a plan and ledger record without calling the API. Use --execute to
delegate to llm_review_priority_queue_v1.0.py after budget and key checks.
"""
...
DEFAULT_QUEUE = BASE_DIR / "评估测试" / "candidate_quality_v2.20_200samples_precision_gated" / "优先复核候选队列_v1.0.csv"
DEFAULT_OUTPUT = BASE_DIR / "评估测试" / "candidate_quality_v2.20_200samples_precision_gated" / "deepseek优先复核结果_budget_guarded_v1.0.csv"
DEFAULT_LEDGER = BASE_DIR / "评估测试" / "deepseek_budget_guard_v1.0" / "deepseek_budget_ledger_v1.0.csv"
DEFAULT_LOG_DIR = BASE_DIR / "评估测试" / "deepseek_budget_guard_v1.0" / "logs"
DEFAULT_PLAN_DIR = BASE_DIR / "评估测试" / "deepseek_budget_guard_v1.0" / "plans"
REVIEW_SCRIPT = SCRIPTS_DIR / "llm_review_priority_queue_v1.0.py"
...
def select_rows(args: argparse.Namespace) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    ...
    if args.resume:
        done = done_keys(args.output_csv)
        pending = [
            row for row in rows
            if (
                row.get("sample_id", ""),
...
def estimate_chars(row: dict[str, str]) -> int:
    fields = [
        ...
        row.get("source_page", ""),
        row.get("value_extraction_method", ""),
        row.get("review_reason", ""),
        row.get("source_text", "")[:1400],
    ]
    return sum(len(str(item)) for item in fields) + 700


def estimate_plan(rows: list[dict[str, str]], batch_size: int, max_output_tokens: int, input_cost: float, output_cost: float) -> dict[str, Any]:
    batches = math.ceil(len(rows) / max(batch_size, 1)) if rows else 0
    # Chinese ESG text is dense. This is an estimate for budget guarding, not an invoice.
...
def main() -> None:
    ...
    parser.add_argument("--execute", action="store_true", help="Actually call DeepSeek after dry-run budget checks")
    ...
    plan_json = args.plan_dir / f"deepseek_priority_review_plan_{args.run_id}.json"
    plan_payload = {
        ...
        "spent_before_usd": round(spent_before, 6),
        "estimate": estimate,
        "pricing_note": "Planning estimate only; pass explicit unit prices if billing changes.",
        "command": command,
    }
    ...
    if not args.execute:
        status = "dry_run_only"
        message = "use --execute to call DeepSeek after budget checks"
        append_ledger(args.ledger, ledger_payload(args, status, len(selected), estimate, spent_before, plan_json, command, message))
    elif estimate["rows_pending"] == 0:
        status = "no_pending_rows"
        message = "nothing to review"
        append_ledger(args.ledger, ledger_payload(args, status, len(selected), estimate, spent_before, plan_json, command, message))
    elif not has_deepseek_key():
        status = "missing_api_key"
        message = "DeepSeek API key not configured"
        append_ledger(args.ledger, ledger_payload(args, status, len(selected), estimate, spent_before, plan_json, command, message))
    elif spent_before + float(estimate["estimated_cost_usd"]) > args.budget_usd:
        status = "budget_blocked"
        message = "estimated budget would be exceeded"
        append_ledger(args.ledger, ledger_payload(args, status, len(selected), estimate, spent_before, plan_json, command, message))
    else:
        log_path = args.log_dir / f"deepseek_priority_review_{args.run_id}.log"
        try:
            ...
            message = "DeepSeek priority review timed out"
            append_ledger(args.ledger, ledger_payload(args, status, len(selected), estimate, spent_before, plan_json, command, message, log_path))
    ...
    print(json.dumps({
        ...
        "plan_json": str(plan_json),
        "selected_rows": len(selected),
        "pending_rows": estimate["rows_pending"],
        "estimated_cost_usd": estimate["estimated_cost_usd"],
        "spent_before_usd": round(spent_before, 6),
