...
**免费公网演示：** [https://zhixi-lvjian-esg.onrender.com](https://zhixi-lvjian-esg.onrender.com)

免费实例长时间无访问时会休眠，首次打开可能需要等待冷启动。该地址用于功能展示与复核，不作为生产级持久化服务。
...
- SQLite 演示数据库与本地/Docker持久化运行目录，可在后续上传报告时增量扩展；免费公网实例的运行目录不持久。
...
| 公网只读验收 | 17/17 通过 | 首页、安全响应头、核心接口、检索、导出、趋势、对比与异常参数 |
| 公网上传链路 | HTTP 202、任务成功、39 行结果 | 非评估冒烟样本，仅验证上传、执行与入库，不参与准确率计算 |
...
![公网分析总览与原始文件挂载边界](测试证据/工程冻结前修复验收/公网_分析总览事实边界.png)

### 报告上传与解析

![公网上传与运行期持久化边界](测试证据/工程冻结前修复验收/公网_报告解析持久化边界.png)

### 趋势分析与企业对比

![公网企业对比与机器等级边界](测试证据/工程冻结前修复验收/公网_企业对比机器边界.png)

### 公网部署验收

![公网证据原文未挂载边界](测试证据/工程冻结前修复验收/公网_证据原文未挂载边界.png)

![移动端公网分析总览](测试证据/公网部署验收/移动端_公网分析总览.jpg)
...
仓库根目录的 `render.yaml` 将服务明确配置为 Render 免费 Web Service，不创建持久磁盘。当前 Docker 服务已部署至 [https://zhixi-lvjian-esg.onrender.com](https://zhixi-lvjian-esg.onrender.com)，可直接复核分析总览、全局检索、报告上传、任务执行、指标查询、证据回溯、趋势分析、企业对比和结果导出。

仓库内 `测试证据/公网部署验收/` 保存了只读接口验收、非评估上传链路验收及桌面端、移动端截图。只读验收 17/17 通过；上传冒烟测试返回 HTTP 202，任务状态为 `succeeded` 并写入 39 行结果。该 PDF 是专门构造的非评估功能样本，测试不计算也不宣称准确率，完成后通过重新部署恢复公开演示种子。

免费实例用于竞赛展示和功能核验，存在以下边界：

- 长时间没有入站请求时可能休眠，下一次访问需要等待冷启动；
- 运行数据库需要重新初始化时会从只读种子恢复，以便继续复核总览、检索、趋势和企业对比；
...
- 三年数据冻结事实：`正式数据产物/P531哈希与幂等性验证结果.json`
- 测试证据：`测试证据/`
